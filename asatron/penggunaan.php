<?php  
	...
	@$jumlah_pembayaran = ($jumlah_meter*$tarif_perkwh);
	...
	if (isset($_POST['bcari'])) {
		$text = $_POST['tcari'];
		$cari = "WHERE id_pelanggan LIKE '%$text%' OR id_penggunaan LIKE '%$text%' OR meter_awal LIKE '%$text%' OR meter_akhir LIKE '%$text%' OR tahun LIKE '%$text%' OR nama_pelanggan LIKE '%$text%' OR nama_manager LIKE '%$text%'";
	}else{
									...
									<tbody>
										<?php  
											...
											if ($data=="") {
												...
												foreach ($data as $r) {
													$cek = $aksi->cekdata("tagihan WHERE id_pelanggan = '$r[id_pelanggan]' AND bulan = '$r[bulan]' AND tahun = '$r[tahun]' AND status = 'Belum pembayaran'");
												$no++; ?>
